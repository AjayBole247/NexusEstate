...
async def restrict_internal_access(request: Request, call_next):
    ...
    # We restrict to localhost to satisfy the requirement "not the public internet"
...
async def predict_valuation(req: PropertyValuationRequest):
    """
    Predicts the true market value of a property using a Mock Regression formula.
    """
    try:
        ...
        f = req.features
        # X = [city, bedrooms, bathrooms, square_feet, year_built, pool, gym, clubhouse, sports, metro, bus, highway]
        input_data = [[
            f.city, f.bedrooms, f.bathrooms, f.square_feet, f.year_built,
            f.has_swimming_pool, f.has_gym, f.has_clubhouse, f.has_sports_ground,
...
async def analyze_fraud(req: FraudAnalysisRequest):
    ...
    where the price deviates more than 3 standard deviations from the neighborhood mean.
    """
    try:
...
async def calculate_desirability(req: PropertyValuationRequest):
    ...
    Combines Amenities and Proximity to calculate a "Best Value" score out of 100.
    """
    try:
        f = req.features
        if f is None:
        ...
        # Amenities weighting (Gyms and Pools add high desirability in Indian cities)
        score += (f.has_gym * 10)
        score += (f.has_swimming_pool * 8)
        score += (f.has_clubhouse * 5)
        score += (f.has_sports_ground * 5)
        
        # Proximity weighting (closer is better)
        # Ideal metro distance is < 1km. Max penalty at > 10km
        ...
        return {
            ...
            "breakdown": {
                "base": 50,
                "amenity_bonus": (f.has_gym*10) + (f.has_swimming_pool*8) + (f.has_clubhouse*5) + (f.has_sports_ground*5),
...
if __name__ == "__main__":
    import uvicorn
    # Bind only to localhost to restrict external access natively
